...
export default (agenda) => {
    ...
    agenda.define(jobs.auth.UPDATE_USER_LAST_LOGIN, options, async (job, done) => {
        ...
        if (!updated) {
            Logger.error('Failed to update the lastLogin of user: ' + user.username, {
                label: 'JOB',
                user: user.username
            });
            return done(new Error('Failed to update the lastLogin of user: ' + user.username));
        }
